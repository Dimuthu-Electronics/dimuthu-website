...
const WHATSAPP_NUMBER = "94711114767";
...
export default function BookAppointmentPage() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    lines.push(`Service: ${formData.serviceType}`);
    if (formData.preferredDate) lines.push(`Preferred date: ${formData.preferredDate}`);
    if (formData.preferredTime) lines.push(`Preferred time: ${formData.preferredTime}`);
    if (formData.description) lines.push("", `Details: ${formData.description}`);
      ...
      <div className="min-h-[60vh] flex items-center justify-center bg-light-gray">
        <div className="max-w-md mx-auto text-center px-4">
          ...
          <p className="mt-3 text-muted">
            Thanks, {formData.name}. We&apos;ve opened WhatsApp with your appointment details, just
            tap send and we&apos;ll reply to confirm your slot.
          </p>
          <p className="mt-3 text-sm text-muted">
            WhatsApp didn&apos;t open?{" "}
            <a href={waUrl} target="_blank" rel="noopener noreferrer" className="text-[#25D366] font-medium hover:underline">
      ...
      <section className="bg-gradient-to-r from-primary-dark to-primary text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
          ...
          <p className="mt-3 text-blue-100 max-w-2xl text-lg">
            Tell us what you need and we&apos;ll arrange it over WhatsApp. Or just walk in, we&apos;re
            at 266 Negombo Road, Ja-Ela.
          </p>
        </div>
      </section>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-sm p-6 md:p-8">
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label htmlFor="date" className="block text-sm font-medium text-foreground mb-1.5">Preferred Date (optional)</label>
                      <input type="date" id="date" value={formData.preferredDate} onChange={(e) => setFormData({ ...formData, preferredDate: e.target.value })} className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors" />
                    </div>
                    <div>
                      <label htmlFor="time" className="block text-sm font-medium text-foreground mb-1.5">Preferred Time (optional)</label>
                      <input type="time" id="time" value={formData.preferredTime} onChange={(e) => setFormData({ ...formData, preferredTime: e.target.value })} className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors" />
                    </div>
                  </div>

                  <div>
                    <label htmlFor="description" className="block text-sm font-medium text-foreground mb-1.5">Describe the problem or what you need</label>
                  ...
                  <button type="submit" className="w-full py-3 bg-primary text-white font-semibold rounded-lg hover:bg-primary-dark transition-colors inline-flex items-center justify-center gap-2">
                  ...
                  <p className="text-xs text-muted text-center">
                    This opens WhatsApp with your details ready to send. We&apos;ll reply to confirm
                    your appointment.
                  </p>
                </form>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                ...
                <div className="space-y-3">
                  ...
                  <a href="https://wa.me/94711114767" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[#25D366] font-medium hover:opacity-80 transition-opacity">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                    </svg>
                    071 111 4767
